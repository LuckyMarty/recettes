...
export default function RecipeEditor({ recipe, onChange }) {
  ...
  // Tags management
  const presetTags = ['Entrée', 'Plat', 'Dessert', 'Végétarien', 'Vegan', 'Sucré', 'Salé']
  ...
  function toggleTag(tag) {
    ...
    const next = has ? recipe.tags.filter((t) => t !== tag) : [...(recipe.tags || []), tag]
    onChange({ tags: next })
  }
  ...
  function onStepDrop(e, idx) {
    e.preventDefault()
    const from = dragIndex.current != null ? dragIndex.current : Number(e.dataTransfer.getData('text/plain'))
    const to = idx
  ...
  function onIngredientDrop(e, idx) {
    e.preventDefault()
    const from = ingDragIndex.current != null ? ingDragIndex.current : Number(e.dataTransfer.getData('text/plain'))
    const to = idx
    if (from === to || from == null) {
      setIngDragOverIndex(null)
      return
  ...
  return (
    <div className="editor">
      <h3>📝 Nom de la recette
        <input value={recipe.title} onChange={(e) => updateField('title', e.target.value)} placeholder="ex: Tarte aux pommes" />
      </h3>

          {/* Tags - moved under title */}
          <div>
            <h4 style={{marginTop:8}}>🏷️ Catégories</h4>
            ...
            <div className="tag-add-row">
              <input placeholder="Ajouter une catégorie personnalisée" value={newTag} onChange={(e) => setNewTag(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addNewTag() } }} />
      ...
      <h3>💬 Sous-titre
        <input value={recipe.subtitle} onChange={(e) => updateField('subtitle', e.target.value)} placeholder="ex: Dessert familial traditionnel" />
      </h3>
      ...
      <div className="row">
        <label>
          ...
          Personnes
          <input value={recipe.servings} onChange={(e) => updateField('servings', e.target.value)} placeholder="ex: 4" />
        </label>
        ...
        <label>
          ...
          Préparation
          <input value={recipe.prepTime} onChange={(e) => updateField('prepTime', e.target.value)} placeholder="ex: 20 min" />
        </label>
        ...
        <label>
          ...
          Cuisson
          <input value={recipe.cookTime} onChange={(e) => updateField('cookTime', e.target.value)} placeholder="ex: 45 min" />
        </label>
      </div>
        ...
        <div className="image-controls">
          ...
          {recipe.image && (
            <div className="image-preview-row">
              ...
              <button className="btn btn-ghost" onClick={removeImage} title="Supprimer la photo">🗑️ Supprimer</button>
            </div>
          )}
        </div>

        <h3>🥕 Ingrédients</h3>
        <p className="muted-text">Utilisez les flèches <strong>▲</strong> et <strong>▼</strong> pour changer l'ordre.</p>
        {recipe.ingredients.map((ing, idx) => (
          ...
          >
            <div className="step-controls">
              <span
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'ArrowDown' && idx < recipe.ingredients.length - 1) moveIngredient(idx, 1)
                }}
              >
                ≡
              </span>
            ...
            <textarea
              ...
              rows={1}
              value={ing}
              onChange={(e) => { updateIngredient(idx, e.target.value); autoResize(e) }}
              onInput={autoResize}
              onKeyDown={(e) => e.stopPropagation()}
              placeholder="ex: 250g de farine"
            />
                        <button className="btn btn-icon" onClick={() => removeIngredient(idx)} aria-label={`Supprimer ingrédient ${idx}`} title="Supprimer cet ingrédient">✕</button>
          </div>
        ))}
        ...
        {recipe.steps.map((s, idx) => (
          ...
          >
            <div className="step-controls">
              <span
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'ArrowDown' && idx < recipe.steps.length - 1) moveStep(idx, 1)
                }}
              >
                ≡
              </span>
            ...
            <textarea
              ...
              value={s}
              onChange={(e) => { updateStep(idx, e.target.value); autoResize(e) }}
              onInput={autoResize}
              onKeyDown={(e) => e.stopPropagation()}
              placeholder="ex: Préchauffer le four à 180°C..."
            />
            <button className="btn btn-icon" onClick={() => removeStep(idx)} aria-label={`Supprimer étape ${idx}`} title="Supprimer cette étape">✕</button>
          </div>
        ))}
